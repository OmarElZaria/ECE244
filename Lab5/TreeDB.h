...
private:
   ...
   // You will need to add additional private functions
    DBentry* help_find(string name, TreeNode* current);
    bool insert_bt(DBentry* newEntry, TreeNode* root); 
    void help_clear(TreeNode* root);
    void help_count(TreeNode* current, int& count) const;
public:
